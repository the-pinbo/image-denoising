...
def vsualize_predictions(model, noise_img_path, clean_img_dir):
    noise_img = cv2.imread(noise_img_path)
    prediction = model.predict(noise_img.reshape(-1, 256, 256, 3)/255)
    plt.figure(figsize=(15, 15))
    name = noise_img_path.split('/')[-1]
    clean_img_path = clean_img_dir + "/" + name
